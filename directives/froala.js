    ...
    'froalaConfig', 'Model', function (froalaConfig, Model) {
      ...
      return {
        ...
        scope: {
          ...
          staticOptions: '@froalaOptions', // Alias of stratusFroala, but for
        ...
        link: function (scope, element, attrs, ngModel) {
          ...
          ctrl.createEditor = function (froalaInitOptions) {
            ...
            if (!ctrl.editorInitialized) {
              ...
              ctrl.options = angular.extend({}, defaultConfig, froalaConfig,
                scope.froalaOptions, scope.staticOptions, froalaInitOptions)
              ...
              // Registering events before initializing the editor will bind
              // the initialized event correctly.
              for (var eventName in ctrl.options.events) {
                if (ctrl.options.events.hasOwnProperty(eventName)) {
                  ctrl.registerEventsWithCallbacks(eventName,
                    ctrl.options.events[eventName])
                }
              }

              ctrl.froalaElement = element.froalaEditor(ctrl.options)
                .data('froala.editor').$el
              ctrl.froalaEditor = angular.bind(element, element.froalaEditor)
              ctrl.initListeners()

              // assign the froala instance to the options object to make
              // methods available in parent scope
          ...
          ctrl.updateModelView = function () {
            ...
            if (specialTag) {
              ...
              for (var i = 0; i < attributeNodes.length; i++) {
                ...
                if (ctrl.options.angularIgnoreAttrs &&
