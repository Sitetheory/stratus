  ...
  Stratus.Components.Permissions = {
    ...
    controller: function ($scope, $timeout, $attrs, $http) {
      ...
      $scope.processSelectAction = function () {
        ...
        if (_.isObject($ctrl.ngModel)) {
          if (!Stratus.Environment.get('production')) {
            console.warn('ngModel is not an object');
          }
          $ctrl.ngModel.data.permissions = $scope.permissionSelected;
        }
      };
      ...
      $scope.processDataSubmit = function () {
        if ($scope.userRoleSelected && $scope.userRoleSelected.name) {
          return {
            identity: { role: $scope.userRoleSelected.id },
            asset: { asset: $scope.contentSelected.assetType, id: $scope.contentSelected.id },
            permissions: $scope.permissionSelected
          };
        }
        return {
          identity: { user: $scope.userRoleSelected.id },
          asset: { asset: $scope.contentSelected.assetType, id: $scope.contentSelected.id },
          permissions: $scope.permissionSelected
        };
      };
