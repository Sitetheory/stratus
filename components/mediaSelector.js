  ...
  Stratus.Components.MediaSelector = {
    ...
    controller: function (
      ...
      $scope.$watch('draggedFiles', function (data) {
        if (_.isUndefined($scope.$ctrl.ngModel) ||
          !_.isEqual($scope.draggedFiles, $scope.$ctrl.ngModel)) {
      ...
      $scope.deleteFromMedia = function (fileId) {
        ...
        $mdDialog.show(deleteMedia).then(function () {
          media.deleteMedia(fileId).then(function (response) {
            // check if deleted media is dragged above,then remove from
            // selected list
          ...
          for (var i = 0; i < $scope.collection.models.length; i++) {
            if ($scope.draggedFiles && $scope.draggedFiles.length > 0) {
              for (var j = 0; j < $scope.draggedFiles.length; j++) {
                if ($scope.draggedFiles[j].id ===
                  $scope.collection.models[i].data.id) {
                  $scope.collection.models[i].data.selectedClass = true
                }
              }
            } else {
      ...
      $scope.$watch('files', function (files) {
        if (files !== null) {
          ...
          // make files array for not multiple to be able to be used in
          // ng-repeat in the ui
          ...
          for (var i = 0; i < files.length; i++) {
            $scope.errorMsg = null;
            (function (f) {
              // setTimeout(function(){ promises.push($scope.saveMedia(f)); },
              // 3000);
      ...
      $scope.saveMedia = function (file) {
        ...
        file.upload.progress(function (evt) {
          // setTimeout(function(){ file.progress = Math.min(100,
          // parseInt(100.0 * evt.loaded / evt.total)); }, 5000);
      ...
      $scope.addDeleteMedia = function (selectedStatus, fileId, $event) {
        // if selected status is true,remove from draggedFiles and add minus
        // icon
